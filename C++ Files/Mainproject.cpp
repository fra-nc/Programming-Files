...
public:
    ...
    Node()  
    {
        ...
        next = NULL;
    } // Constructor with parameters with provided values
    ...
    {
        double Mean_Score = (Aggregate) / 11.0;
...
public:
    ...
    void push(Node *n)
    {
        ...
        else if (n->Aggregate > top->Aggregate)
        {
            // If the new node's aggregate is greater than the top node's aggregate, make the new node as top.

            n->next = top; //This line assigns the next pointer of the node n to point to the current top node of the stack.

            top = n; //This line updates the top pointer of the stack to point to the newly added node n, effectively making n the new top node of the stack.
        ...
        {
            // Otherwise, find the appropriate position to insert the new node based on aggregate marks.
            Node *temp = top;
            while (temp->next != NULL && temp->next->Aggregate > n->Aggregate)
            {
                temp = temp->next;
            }
            n->next = temp->next;
